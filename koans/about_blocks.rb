...
class AboutBlocks < Neo::Koan
  def method_with_block
    result = yield # выполняет переданный блок кода
    result
  end
  ...
  def test_blocks_can_take_arguments
    method_with_block_arguments do |argument| #argument равен тому, что передаст yield
  ...
  def method_with_explicit_block(&block) # метод ожидает получить блок кода
    block.call(10)
  end
  ...
  def test_methods_can_take_an_explicit_block_argument
    ...
    add_one = lambda { |n| n + 1 }
    assert_equal 11, method_with_explicit_block(&add_one) # передача блока
  end
